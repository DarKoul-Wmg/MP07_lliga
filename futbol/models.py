...
class Partit(models.Model):
    ...
    def gols_local(self):
        return self.event_set.filter(jugador__equip=self.equip_local, 
                                     tipus_esdeveniment="gol").count()
    
    def gols_visitant(self):
        return self.event_set.filter(jugador__equip=self.equip_visitant,
                                     tipus_esdeveniment="gol").count()
